...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const resp = await apiLogin(email, dob, role);
        if (resp && resp.success) {
        // store new JSON user and also keep legacy keys for compatibility
        localStorage.setItem("user", JSON.stringify(resp.user));
        try {
          const name = resp.user?.name || resp.user?.email || '';
          const rawRole = resp.user?.role || role;
          localStorage.setItem('userName', name);
          // map backend 'staff' to frontend 'faculty'
          localStorage.setItem('userType', rawRole === 'staff' ? 'faculty' : rawRole);
        } catch (e) {
          // ignore storage errors
        }
        // Redirect based on selected role: coordinators -> coordinator page, admin -> admin, students -> seats
        let dest = '/seats';
        if (role === 'coordinator') dest = '/coordinator';
        else if (role === 'admin') dest = '/admin';
        else if (resp.user?.role === 'staff' && role === 'student') {
          // edge: backend returned 'staff' but frontend selected student; keep student destination
          dest = '/seats';
        } else if (resp.user?.role === 'staff' && (role === 'admin' || role === 'coordinator')) {
          // ensure admin/coordinator selection overrides
          dest = role === 'coordinator' ? '/coordinator' : '/admin';
        }
        navigate(dest);
      } else {
  ...
  .role-outline .role-text { color: #0f172a !important; font-weight:600 }
      ...
      <Card className="vibrant-card w-full max-w-md shadow-2xl border-0">
          ...
          <CardDescription className="text-base">Login with your email and DOB (YYYY-MM-DD)</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="dob" className="text-sm font-semibold label-vibrant">
                Date of Birth
              </Label>
              <Input
                id="dob"
